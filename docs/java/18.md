...
Static 关键字可以与 class，variable，method 和 block 一起使用。静态成员属于类而不是特定实例，这意味着如果您将成员设为静态，则可以在不使用对象的情况下访问它。我们举一个例子来理解这个：
...
在此示例中，String 变量是非静态的，整数变量是 Static。正如您在输出中看到的那样，两个对象的非静态变量是不同的，但静态变量在它们之间共享，这就是对象`ob2`对静态变量所做的更改在两个对象中反映的原因。
