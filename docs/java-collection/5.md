...
2） **add（int index，Object o）**：它将对象 o 添加到给定索引处的数组列表中。

```
obj.add(2, "bye");
```

它将字符串再见添加到数组列表的第二个索引（数组列表以索引 0 开头的第三个位置）。
...
函数 get 将返回存储在第 3 个位置（索引 2）的字符串，并将分配给字符串“str”。我们已将返回的值存储在字符串变量中，因为在我们的示例中，我们已经定义了 ArrayList 是 String 类型。如果您有整数数组列表，则返回的值应存储在整数变量中。
...
9） **boolean contains（Object o）**：它检查给定对象 o 是否存在于数组列表中，如果它在那里则返回 true，否则返回 false。
...
10） **clear（）：**它用于一次性删除数组列表的所有元素。下面的代码将删除其对象为 obj 的 ArrayList 的所有元素。
