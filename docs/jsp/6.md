...
`isErrorPage`属性还有另一种用法 - [异常隐式对象](https://beginnersbook.com/2013/11/jsp-implicit-object-exception-with-examples/)只能用于`isErrorPage`设置为`true`的那些页面。如果值为`false`，则页面不能使用异常隐式对象。
